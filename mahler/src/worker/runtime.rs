...
fn try_subscribe(
    ...
    if let Some((args, sensor)) = sensors.at(pointer.as_str()) {
        ...
        let stream = sensor.create_stream(&context)?;
    ...
    tokio::spawn(
        async move {
            ...
            let res = {
                ...
                {
                    let system = system_writer.write().await;
                    let mut all_paths = Vec::new();
                    collect_paths(system.inner_state(), Pointer::root(), &mut all_paths);

                    for path in all_paths {
                        try_subscribe(&path, &sensors, &mut sensor_streams)?;
                    }
                }
