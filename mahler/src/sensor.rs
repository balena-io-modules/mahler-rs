...
///
/// A SensorHandler is any function that accepts zero or more "[context extractors](`FromContext`)" as
/// arguments and returns something that can be converted into a [`Stream`] of values.
///
/// Unlike task handlers, sensor handlers only have access to context extractors (Args, Path),
/// not system state extractors (View, System).
pub trait SensorHandler<T, U>: Clone + Sync + Send + 'static {
    ...
    fn call(&self, context: &Context) -> Result<SensorStream>;
}
...
macro_rules! impl_sensor_handler {
    (
            ...
            $($ty: FromContext + Send,)*
        {
            fn call(&self, context: &Context) -> Result<SensorStream> {
                $(
                    let $ty = $ty::from_context(context)?;
                )*
...
type SensorFactory = Arc<dyn Fn(&Context) -> Result<SensorStream> + Send + Sync>;
...
impl Sensor {
        ...
        H: SensorHandler<T, U>,
    {
        Sensor {
            ...
            factory: Arc::new(move |context| handler.call(context)),
        }
    }

    /// Create a sensor stream for the given context
    pub fn create_stream(&self, context: &Context) -> Result<SensorStream> {
